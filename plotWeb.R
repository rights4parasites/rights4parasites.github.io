plotWeb <- function (web, method = "cca", empty = FALSE, labsize = 1, 
                     ...
                     high.abun2 = NULL, high.abun.col2 = "red") 
{
  ...
  wdown=0.1
  plot(0, type = "n", 
       xlim = range(wleft, wright), 
       ylim = range(wdown/ybig,wup * ybig), 
  ...
  for (i in 1:n.pred) {
    ...
    if (!is.null(high.abun)) {
      rect(pred_x + pred_prop[i] - diffh[i], pred_y - y_width, 
           pred_x + pred_prop[i], pred_y + y_width, col = high.abun.col)
    }
